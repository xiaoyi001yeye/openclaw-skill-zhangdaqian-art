...
class ListPageParser(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if (tag == "div" and ("listItem" in class_name or "list-item" in class_name)) or tag == "li":
            self.in_list_item = True
            self.current_item = {}
        elif self.in_list_item and (tag == "div" and ("title" in class_name or "name" in class_name)) or tag == "h3":
            self.in_title = True
        elif self.in_list_item and tag == "a" and (self.in_title or "/paimai-art" in attrs_dict.get("href", "")):
            href = attrs_dict.get("href", "")
            if "/paimai-art" in href:
                self.current_item["detail_url"] = urljoin(BASE_URL, href)
                # 提取ID
                match = re.search(r"paimai-art(\d+)", self.current_item["detail_url"])
                if match:
                    self.current_item["id"] = match.group(1)
        elif self.in_list_item and tag == "div" and ("artist" in class_name or "author" in class_name):
            self.in_artist = True
        elif self.in_list_item and tag == "div" and ("company" in class_name or "auction-house" in class_name):
            self.in_company = True
        elif self.in_list_item and tag == "div" and ("time" in class_name or "date" in class_name):
            self.in_time = True
        elif self.in_list_item and tag == "div" and ("estimate" in class_name or "valuation" in class_name):
            self.in_estimate = True
        elif self.in_list_item and tag == "div" and ("price" in class_name or "hammer-price" in class_name):
            self.in_price = True
        elif self.in_list_item and tag == "div" and ("lot" in class_name or "lot-number" in class_name):
            self.in_lot = True
...
def main():
    parser = argparse.ArgumentParser(description="雅昌艺术网艺术品抓取工具（curl版）")
